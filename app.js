...
var tempTarget = {
  ...
  function explode(target){
    // tempTarget.x = mouse.x;
    // tempTarget.y = mouse.y;

    for(var i = 0; i<360; i+=10){
        radius = 50

        let x = mouse.x + Math.cos(i*(Math.PI/180)) * (radius* Math.random())
        let y = mouse.y + Math.sin(i*(Math.PI/180)) * (radius* Math.random())
